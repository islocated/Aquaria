...
If a page happens to be a single packet that was begun on a previous page, and spans to the next page (in the case of a three or
...
int ogg_stream_init( ogg_stream_state* os, int serialno )
{
	if( os )
	{
		...
		/* Real world testing shows a max of about 4300 needed here for vorbis - _os_body_expand() allocates more on demand if needed */
		os->body_storage = 6 * 1024;
		os->body_data = ( unsigned char* )_ogg_malloc( os->body_storage * sizeof( *os->body_data ) );

		/* Real world testing shows a max of about 96 needed here for vorbis - _os_lacing_expand() allocates more on demand needed */
...
/* checksum the page */
/* Direct table CRC; note that this will be faster in the future if we perform the checksum silmultaneously with other copies */
...
int ogg_stream_packetin( ogg_stream_state* os, ogg_packet* op )
{
	...
	if( os->body_returned )
	{
		...
		if( os->body_fill )
		{
			memmove( os->body_data, os->body_data + os->body_returned, os->body_fill );
		}
	...
	/* Copy in the submitted packet.  Yes, the copy is a waste; this is	the liability of overly clean abstraction for the time being.  It
...
/* This will flush remaining packets into a page (returning nonzero), even if there is not enough data to trigger a flush normally
   ...
   (and *not* ogg_stream_flush) unless you specifically need to flush an page regardless of size in the middle of a stream. */
...
{
	/* The last part of decode. We have the stream broken into packet segments.  Now we need to group 
	them into packets (or return the out of sync markers) */
