...
static const unsigned char mask8B[]=
{
...
{
	memset( b, 0, sizeof( *b ) );
	b->buffer = ( unsigned char* )_ogg_malloc( BUFFER_INCREMENT );
	b->ptr = b->buffer;
	b->buffer[0] = '\0';
...
void oggpack_write( oggpack_buffer* b, unsigned long value, int bits )
{
	...
	{
#ifdef	__SSE__
		b->buffer = ( unsigned char* )_ogg_realloc( b->buffer, b->storage + BUFFER_INCREMENT + 4 );
#else
...
long oggpack_look( oggpack_buffer* b, int bits )
{
	...
	{
		/* not the main path */
		if( ( b->endbyte << 3 ) + bits > ( b->storage << 3 ) )
		{
			return( -1 );
		}
	}
